...
contract ShortBox is Initializable, ERC721Upgradeable, ERC721EnumerableUpgradeable, AccessControlUpgradeable, UUPSUpgradeable {
    ...
    function initialize() initializer public {
        __ERC721_init("ShortBox", "SHORTBOX");
    ...
    function mint(address[] memory tos, uint256[] memory tokenIds) public onlyRole(MINTER_ROLE) {
        ...
        require(tokenIds.length <= mintLimit[msg.sender], "overlimit");
        mintLimit[msg.sender] -= tokenIds.length;
        for (uint256 i = 0; i < tokenIds.length; ++i){
            _safeMint(tos[i], tokenIds[i]);
        }
    }

    function mintByDeadline(address[] memory tos, uint256[] memory tokenIds, uint256 blockNum) public onlyRole(MINTER_ROLE){
    ...
    function burnByDeadline(uint256[] memory tokenIDs, uint256 blockNum) public onlyRole(MINTER_ROLE){
        require(block.number <= blockNum, "overblock!");
        burn(tokenIDs);
    }
